...
public class ItemUploadActivity extends AppCompatActivity {
    Button upload;
    EditText name,prod,exp,amt,cost,discount;
    ...
    DatabaseReference databaseItems,databaseItems_1,deals;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        discount = findViewById(R.id.itemdiscount);
        deals = FirebaseDatabase.getInstance().getReference("Deal Of the day");

        databaseItems = FirebaseDatabase.getInstance().getReference("Categories");
        databaseItems_1 = FirebaseDatabase.getInstance().getReference("Products");
    ...
    void uploadData(){
        ...
        int offer = Integer.parseInt(disc);

        String id = databaseItems.push().getKey();
        //
        String id_1 = databaseItems_1.push().getKey();
        //
        Items item = new Items(id, itmname, proddate, expdate, amount, unit, category, price, imageurl,disc);
        databaseItems = databaseItems.child(category);
        databaseItems.child(id).setValue(item);
        //
        databaseItems_1.child(id_1).setValue(item);
        if(offer>=30)
        {Toast.makeText(getApplicationContext(),"Added to deal",Toast.LENGTH_LONG).show();
            deals.child(id).setValue(item);
        }
